...
def fetch_uniprot_mapping(alias_list, fr="ACC,ID", to="P_ENTREZGENEID"):
    ...
    if r.status_code != requests.codes.ok:
        raise Exception(
            f"Failed to request mapping data with status={r.status_code} and reason={r.text}"
        )
    ...
    if r.status_code != requests.codes.ok:
        raise Exception(
            f"Failed to get mapping data with status={r.status_code} and reason={r.text}"
        )
    ...
    if r.status_code != requests.codes.ok:
        raise Exception(
            f"Failed to get missing data with status={r.status_code} and reason={r.text}"
        )
...
def fetch_uniprot_to_entrez(alias_list):
    uniprot_alias_list = [
        a[1]
        for a in [a.split("uniprot knowledgebase:") for a in alias_list]
        if len(a) > 1
    ]
