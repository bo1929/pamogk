def pandas_to_latex_table(df, row_name, col_name, file_path):
    ...
    # col setup
    table_str_full += (
        "\\begin{tabular}{|p{2cm}|" + "p{2cm}|" * col_size + "}\n\\hline\n"
    )
    table_str_full += (
        f"\\textbf{{${row_name}$/${col_name}$}}"
        + " & \\textbf{0}" * len(col_headers)
        + " \\\\ \\hline\n"
    )

    # rows
    for row_header in row_headers:
        table_str_row_value = " & ".join(
            str(df[col_header][row_header]) for col_header in col_headers
        )
