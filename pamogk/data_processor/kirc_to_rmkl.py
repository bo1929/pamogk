...
def write_csv(data, key, pat_ids, use_index=False):
    ...
    with open(filepath, "w") as f:
        ...
        fieldnames = ["Patient ID"] + list(
            k for k in key_data[0].keys() if k not in ["Patient ID"]
        )
...
def main():
    ...
    rnaseq_data = transpose_data(
        read_csv("kirc_rna_seq_expression_data.csv"), "Entrez Gene ID", ["Gene Name"]
    )
