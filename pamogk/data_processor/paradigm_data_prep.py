...
class ParadigmDataPrep(object):
    ...
    def find_intersection_patients(self, rs_GE, rs_pat, rp_GE, rp_pat, som_pat):
        ...
        intersection_list = list(
            self.find_intersection_lists(rs_pat_list, rp_pat_list, som_pat_list)
        )
        ...
        with (self.exp_data_dir / fname).open("w") as f:
            ...
            for i, pid in enumerate(pat_ids):
                f.write(
                    "\n"
                    + "\t".join(
                        [str(v) for v in ["-".join(pid.split("-")[:3]), *data[i][:]]]
                    )
                )
...
def main():
    ...
    # Find intersect
    rs_GE, rs_pat_ids, rp_GE, rp_pat_ids, som_patients = exp.find_intersection_patients(
