...
def read_communities_from_txt():
    if not COMM_PATH.exists():
        raise ValueError("Given community path does not exit!")
...
def wrt_communities_to_json(comm_all):
    ...
    ts_edge_map = coll.defaultdict(list)
    id_map = {}
    ...
    for idx, community in enumerate(comm_all["communities"]):
        ...
        comm_data["community_edges"] = {
            edge["@id"]: {"s": n, "t": edge["t"]}
            for n in nodes
            for edge in st_edge_map[n]
            if edge["t"] in nodes
        }
        ...
        comm_map["communities_all"].append(
            {"@id": idx, "name": f"comm{str(idx)}"}
        )  ## , "attr": comm_attr[idx]})
