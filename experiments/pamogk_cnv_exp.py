...
parser.add_argument(
    ...
    / "kirc_data"
    / "unc.edu_KIRC_IlluminaHiSeq_RNASeqV2.geneExp.whitelist_tumor.txt",
)
parser.add_argument(
    "--rp-patient-data",
    "-rp",
...
parser.add_argument(
    "--cnv-patient-data",
    metavar="file-path",
    dest="cnv_patient_data",
    type=str2path,
    help="CNV matrix",
    default=config.DATA_DIR
    / "copy-number-variation"
    / "KIRC.focal_score_by_genes-tcga_entrez.csv",
)
parser.add_argument(
    "--label",
    "-m",
...
class Experiment1(object):
    def __init__(self, args):
        ...
        self.get_cnv_pw_path = (
            lambda pw_id, cnv_type: self.kernel_dir
            / f"pamogk-cnv-{cnv_type}-pw_id={pw_id}.gpickle"
        )
    ...
    @timeit
    def find_intersection_patients(
        self, rs_GE, rs_pat, rp_GE, rp_pat, som_pat, cnv_pat
    ):
        ...
        intersection_list = list(
            set(rs_pat_list).intersection(rp_pat_list, som_pat_list, cnv_pat_list)
        )
        ...
        def clean_patient_list_and_ge_data(patients, ge, intersect):
            ...
            to_del = [
                idx for idx, value in enumerate(pat_list) if value not in intersect
            ]
    ...
    def preprocess_cnv_patient_data(self, patients, is_pos=True):
        ...
        for pat_id, patient in patients.items():
            for cnv_type, ent_ids in patient.items():
                patient[cnv_type] = [
                    uid for eid in ent_ids if eid in ent2uni for uid in ent2uni[eid]
                ]
    ...
    def cnv_pathways_save_valid(self, all_pw_map, cnv_type):
        return np.all(
            [self.get_cnv_pw_path(pw_id, cnv_type).exists() for pw_id in all_pw_map]
        )
    ...
    def label_cnv_patient_genes(self, all_pw_map, patients, cnv_type):
        ...
        for pid, patient in patients.items():
            ...
            label_mapper.mark_label_on_pathways(
                f"cnv_{cnv_type}", pid, all_pw_map, genes, self.label
            )
            ind += 1
        log()
    ...
    def run(self):
        ...
        # Find intersect
        intersect = self.find_intersection_patients(
            rs_GE, rs_pat_ids, rp_GE, rp_pat_ids, som_patients, cnv_patients
        )
        rs_GE, rs_pat_ids, rp_GE, rp_pat_ids, som_patients, cnv_patients = intersect
        ...
        labeled_loss_cnv_pw_map = self.label_cnv_patient_genes(
            all_cnv_pw_map, cnv_patients, "loss"
        )
        cnv_kernels_loss = self.create_cnv_kernels(
            labeled_loss_cnv_pw_map, cnv_patients, "loss"
        )

        all_cnv_pw_map = self.read_pathways()
        labeled_gain_cnv_pw_map = self.label_cnv_patient_genes(
            all_cnv_pw_map, cnv_patients, "gain"
        )
        cnv_kernels_gain = self.create_cnv_kernels(
            labeled_gain_cnv_pw_map, cnv_patients, "gain"
        )

        kernels = np.concatenate(
            (rs_kernels, rp_kernels, som_kernels, cnv_kernels_loss, cnv_kernels_gain)
        )
